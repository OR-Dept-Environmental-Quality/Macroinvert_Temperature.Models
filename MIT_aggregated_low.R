...
#				AGGREGATED taxonomic resolution models = LOW
...
rel.abund<-cal.agg[, c("sample.id", "OTU_low.res", "Count")]
rel.abund <- rel.abund %>% 
	left_join(tot.abund, by = c('sample.id')) 


# calculate RA
rel.abund<-plyr::ddply(.data = rel.abund, c('sample.id', 'OTU_low.res'), 
...
cal.agg_RA <- rel.abund %>%
	select(sample.id, OTU_low.res, rel.abund) %>%
	filter(OTU_low.res != 666) 

# need to crosstab the bug data (turn into a wide format) so that OTUs are columns

# high res
bug.cal.agg_low_wide <- cal.agg_RA %>% 
	pivot_wider(id_cols = sample.id, names_from = OTU_low.res, values_from = rel.abund,
					values_fn = sum) 
bug.cal.agg_low_wide[is.na(bug.cal.agg_low_wide)] <- 0
bug.cal.agg_low_wide <-	column_to_rownames(bug.cal.agg_low_wide, 'sample.id')
...
# high res
spec <- bug.cal.agg_low_wide
env <- env.cal

wa_low.res <- WA(y=spec, x=env, mono=TRUE, tolDW = TRUE, use.N2=TRUE, tol.cut=.01, 
					  check.data=TRUE, lean=FALSE)


wa_low.res # 182 taxa, RMSE (inv/cla) = 2.4/3.0, r2 = 0.645 (both), max bias 7.6/4.9
